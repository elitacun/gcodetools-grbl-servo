...
defaults = {
    'header': """%
(Generated by gcodetools mod by Miamolillo for Pen Plotters)
...
class Postprocessor(object):
    def __init__(self, owner): # 'owner' ahora recibe el 'self' que enviamos arriba
        self.owner = owner     # <--- ESTO ES LO QUE TE PERMITIRÁ LEER LAS OPCIONES
        ...
        self.functions = {
            ...
            "gcode_pen_plotter": self.gcode_pen_plotter,
    ...
    def gcode_pen_plotter(self, parameters):
        import re
        
        # ==========================================
        # CAPTURA DE VALORES DESDE LA INTERFAZ
        # ==========================================
        # Acceso directo al "abuelo" (la extensión principal)
        opts = self.owner.options
        # Leemos los strings de comando
        SERVO_UP = opts.servoupcmd
        SERVO_DOWN = opts.servodowncmd
        # Leemos los floats y los formateamos como comandos G4
        # self.options.servo_delay_up traerá el número (ej: 0.1)
        SERVO_DELAY_UP = "G4 P{:g}".format(opts.servodelayup)
        SERVO_DELAY_DOWN = "G4 P{:g}".format(opts.servodelaydown)
        ...
        for line in self.gcode.split("\n"):
            ...
            if original_line.upper() == "M5":
                if last_state != 'UP':
                    processed_lines.append(SERVO_UP)
                    processed_lines.append(SERVO_DELAY_UP)
                    last_state = 'UP'
                continue

            z_match = re.search(r'(?i)Z([-+]?\d*\.\d+|\d+)', original_line)
            
            if z_match:
                z_value = float(z_match.group(1))
                new_state = 'DOWN' if z_value <= 0 else 'UP'
                
                if new_state != last_state:
                    if new_state == 'DOWN':
                        processed_lines.append(SERVO_DOWN)
                        processed_lines.append(SERVO_DELAY_DOWN)
                    else:
                        processed_lines.append(SERVO_UP)
                        processed_lines.append(SERVO_DELAY_UP)
                    last_state = new_state
                
                clean_line = re.sub(r'(?i)Z[-+]?\d*\.\d+|Z\d+', '', original_line)
                has_xy = "X" in clean_line.upper() or "Y" in clean_line.upper()
                
                if not has_xy:
                    clean_line = re.sub(r'(?i)F\d*\.\d+|F\d+', '', clean_line)
                
                check_content = re.sub(r'\(.*?\)', '', clean_line).strip()
                if "X" in check_content.upper() or "Y" in check_content.upper():
                    processed_lines.append(" ".join(clean_line.split()).strip())
            else:
                if "X0.0000" in original_line and "Y0.0000" in original_line:
                    if last_state != 'UP':
                        processed_lines.append(SERVO_UP)
                        processed_lines.append(SERVO_DELAY_UP)
                        last_state = 'UP'
...
class Gcodetools(inkex.EffectExtension):
    ...
    def export_gcode(self, gcode, no_headers=False):
        if self.options.postprocessor != "" or self.options.postprocessor_custom != "":
            postprocessor = Postprocessor(self) #mod
    ...
    def add_arguments(self, pars):
        ...
        # Argumentos para el Pen Plotter personalizado (miamolillo)
        add_argument("--servoupcmd", type=str, default="M3 S125", help="Comando para subir servo")
        add_argument("--servodowncmd", type=str, default="M3 S0", help="Comando para bajar servo")
        add_argument("--servodelayup", type=float, default=0.1, help="Retardo al subir")
        add_argument("--servodelaydown", type=float, default=0.1, help="Retardo al bajar")
